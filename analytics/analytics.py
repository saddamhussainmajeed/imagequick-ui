...
def chart_templates():
	...
	chart['producer'] = ''
	for event in db.events.find({'producer':{'$exists':True}}):
		if event['event'] == 'play':
...
def chart_voice_format():
	...
	chart.to_excel('files/home/manu/voice_format_analysis.xls')
	return chart
...
def monthly_templates(month,year):
	...
	chart = DataFrame(data,index=index,columns=['play','buy'])
	chart['producer'] = ''
	for event in db.events.find({'producer':{'$exists':True},'date.year':year,'date.month':month}):
		if event['event'] == 'play':
			chart.play[event['template']] += 1
		if event['event'] == 'purchase':
			chart.buy[event['template']] += 1
...
def monthly_format(month,year):
	...
	chart = DataFrame(data,index=index)
	for event in db.events.find({'date.year':year,'date.month':month}):
		if event['format'] is not None:
			if event['event'] == 'play':
				chart.play[event['format']] += 1
			elif event['event'] == 'purchase':	
